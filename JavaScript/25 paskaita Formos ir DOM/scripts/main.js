...
.addEventListener("submit", e =>{
  e.preventDefault();
  console.dir(e);
  const [photo,title, desciption, date, location] = [
    e.target.elements.photo.value,
    e.target.elements.title.value,
    e.target.elements.desciption.value,
    e.target.elements.date.value,
    e.target.elements.location.value
  ];
  console.log(photo,title, desciption, date, location);

  document.querySelector('#generatedCards').innerHTML +=
  ` <div class="card">
  <div>
    <img src="${photo}" alt="cardImages"> 
    ${date && `<p>${date}</p>`}
...
document
.querySelector('#formToTable > form')
.addEventListener("submit", e =>{
  e.preventDefault();
  const [eilutes,stulpeliai] =[
  e.target.elements.eilutes.value,
  e.target.elements.stulpeliai.value
  ];

  const tableContainer = document.createElement('table');
  for(let i = 0; i < eilutes; i++){
    const tableRow = document.createElement('tr');
    for(let i = 0; i < stulpeliai; i++){
      const tableData = document.createElement('td');
      tableRow.append(tableData);
    }
    tableContainer.append(tableRow);
  }
    document.querySelector('#table').append(tableContainer);
});
...
data.movies.forEach(element=>{
  document.querySelector('#movieCards').innerHTML +=
  `
  <div class="cardMovie">
    <div>
      <h1>${element.pavadinimas}</h1>
      <img src="${element.paveiksliukas}" class=img alt="movieImages">
    </div>
    <div>
      <p>Realese date: ${element.leidimoMetai}</p>
      <p>Type: ${element.tipas}</p>
      <p>Length: ${element.trukme} min.</p>
      <p>About: ${element.aprasymas}</p> 
      <p>IMBD rating: ${element.IMDB}</p>
      <p>Director: ${element.rezisierius}</p>
      <p>Actors: ${element.aktoriai}</p>
    </div>
  </div>
  `;
});
...
function sleptiRodyti() {
  var x = document.getElementById("sleptiRodyti");
